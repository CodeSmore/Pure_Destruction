...
public class MeshBuffer : BufferBase {
	...
	void Initialize() {
		...
		DrawSelf (destructible.originalSpriteMaterial);
	...
	void Update() {
		...
		if (destructible.eraseEvents.Count > 0 || destructible.modifiersAdded == true) {
			...
			//foreach(DestructionModifier modifier in destructible.modifiers) {
			//	Max2D.DrawImage(transform, modifier.material, modifier.position + new Vector2(transform.position.x - destructible.transform.position.x, transform.position.y - destructible.transform.position.y), modifier.size, modifier.rotation, transform.position.z + 0.3f);
			//}
